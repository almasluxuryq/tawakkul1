...
export default function CheckoutPage() {
  ...
  const countryOptions: { value: Country; label: string }[] = [
    { value: 'ru', label: '🇷🇺 ' + p.countryRu },
    { value: 'kz', label: '🇰🇿 ' + p.countryKz },
    { value: 'world', label: '🌍 ' + p.countryWorld },
  ]
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-12">
            ...
            <div className="lg:col-span-3 space-y-8">
              <section className="space-y-4">
                <h2 className="text-base font-medium text-white/80">{p.whereDeliver}</h2>
              ...
              <section className="space-y-4">
                <h2 className="text-base font-medium text-white/80 border-b border-white/10 pb-3">{p.contacts}</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="name">{p.fio} *</Label>
                    <Input id="name" {...register('name')} placeholder={p.fioPh} className={errors.name ? inputErrorClass : inputClass} />
                    <FieldError message={errors.name?.message} />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="phone">{p.phone} *</Label>
                    <Input id="phone" type="tel" inputMode="tel" {...register('phone')} placeholder={p.phonePh} className={errors.phone ? inputErrorClass : inputClass} />
                    <FieldError message={errors.phone?.message} />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="messenger">{p.messenger} *</Label>
                    <Input id="messenger" {...register('messenger')} placeholder={p.messengerPh} className={errors.messenger ? inputErrorClass : inputClass} />
                    <FieldError message={errors.messenger?.message} />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="email">{p.email}</Label>
                    <Input id="email" type="email" inputMode="email" {...register('email')} placeholder={p.emailPh} className={errors.email ? inputErrorClass : inputClass} />
                  </div>
                </div>
              </section>

              <section className="space-y-4">
                <h2 className="text-base font-medium text-white/80 border-b border-white/10 pb-3">{p.addressTitle}</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="city">{p.city} *</Label>
                    <Input id="city" {...register('city')} placeholder={p.cityPh} className={errors.city ? inputErrorClass : inputClass} />
                    <FieldError message={errors.city?.message} />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="postalCode">{p.index}</Label>
                    <Input id="postalCode" inputMode="numeric" {...register('postalCode')} placeholder={p.indexPh} className={inputClass} />
                  </div>
                  <div className="space-y-2 sm:col-span-2">
                    <Label htmlFor="address">
                      {country === 'ru' ? p.addrRu : country === 'kz' ? p.addrKz : p.addrWorld} *
                    </Label>
                    <Input id="address" {...register('address')} placeholder={p.addrPh} className={errors.address ? inputErrorClass : inputClass} />
                ...
                <div className="flex items-start gap-3 p-4 bg-white/5 border border-white/10 rounded-lg">
                  ...
                  <div className="text-sm text-white/70 leading-relaxed">
                    {country === 'ru' && p.delivInfoRu}
                    {country === 'kz' && p.delivInfoKz}
                    {country === 'world' && p.delivInfoWorld}
                  </div>
                </div>
              </section>

              <section className="space-y-3">
                <h2 className="text-base font-medium text-white/80 border-b border-white/10 pb-3">{p.paymentTitle}</h2>
                <div className="flex items-start gap-3 p-4 bg-white/5 border border-white/10 rounded-lg">
                  <Info className="h-5 w-5 text-white/50 flex-shrink-0 mt-0.5" />
                  <div className="text-sm text-white/70 leading-relaxed">
                    {country === 'ru' && p.payInfoRu}
                    {country === 'kz' && p.payInfoKz}
                    {country === 'world' && p.payInfoWorld}
                  </div>
                </div>
              </section>
            </div>
            ...
            <div className="lg:col-span-2">
              <div className="lg:sticky lg:top-24 space-y-5 p-5 sm:p-6 bg-white/5 rounded-xl border border-white/10">
                <h2 className="text-lg font-medium">{p.yourOrder}</h2>
                      ...
                      <div key={`${item.productId}-${item.size}-${item.color ?? ''}`} className="flex gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <button type="button" onClick={() => removeItem(item.productId, item.size, item.color)} className="text-white/30 hover:text-red-400 flex-shrink-0" aria-label="×">
                              <Trash2 className="h-3.5 w-3.5" />
                            </button>
                          </div>
                          <p className="text-xs text-white/40 mt-0.5">
                            {p.size}: {item.size}{item.color ? ` · ${colorLabel(language, item.color)}` : ''}
                          </p>
                ...
                <div className="border-t border-white/10 pt-4 space-y-2.5 text-sm">
                  <div className="flex justify-between">
                    <span className="text-white/60">{p.goods}</span>
                    <span>{fmt(totalPriceRUB, t.common.price.rub)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-white/60">{p.delivery}</span>
                    <span className="text-right text-white/70">
                      {country === 'ru' && p.atPickup}
                      {country === 'kz' && `+${fmt(KZ_DELIVERY_FEE, t.common.price.kzt)}`}
                      {country === 'world' && p.negotiate}
                    </span>
                  </div>
                </div>
                ...
                <div className="border-t border-white/10 pt-4">
                  <div className="flex items-end justify-between">
                    <span className="text-base font-medium">{p.payNowLabel}</span>
                  ...
                  {country === 'ru' && (
                    <p className="text-[11px] text-white/40 mt-2">{p.cdekSeparate}</p>
                  )}
                </div>
                ...
                <Button type="submit" disabled={isSubmitting} className="w-full bg-white text-black hover:bg-white/90 py-6 text-base font-medium disabled:opacity-50">
                  {isSubmitting ? '...' : p.placeOrder}
                </Button>
                <p className="text-[11px] text-white/40 text-center leading-relaxed">{p.afterOrderHint}</p>
              </div>
            </div>
          </div>
        </form>
